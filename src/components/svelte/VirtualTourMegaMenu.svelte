<script lang="ts">
  ...
  let {
    currentLang = 'en',
    activeDemo = null,
  }: { currentLang?: string; activeDemo?: 'hub' | 'shell' | 'entity-list-table' | 'exports' | 'versions' | null } = $props();
  ...
  const versionsHref = $derived(`${demoHref}versions`);
  ...
  const demos = $derived([
    ...
    { key: 'versions', href: versionsHref, color: 'rose', icon: '<path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8M3 3v5h5M12 7v5l4 2"/>' },
